...
async def list_entities(page: int = 1, size: int = 20, q: str | None = None):
    query = {}
    return await list_docs("opportunities", page, size, query)

@router.get("/{entity_id}")
async def get_entity(entity_id: str):
    doc = await get_doc("opportunities", entity_id)
    if not doc:
...
async def create_entity(payload: dict, user=Depends(get_current_user)):
    ...
    return await create_doc("opportunities", payload)

@router.patch("/{entity_id}")
async def update_entity(entity_id: str, updates: dict):
    return await update_doc("opportunities", entity_id, updates)

@router.delete("/{entity_id}")
async def delete_entity(entity_id: str):
    return await delete_doc("opportunities", entity_id)
